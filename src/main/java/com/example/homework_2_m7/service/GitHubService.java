...
public class GitHubService {
    ...
    public List<BranchResult> fetchAllBranchesForOneRepo(String owner, String repo) {
        try {
...
//        try {
//            String json = gitClient.makeGetRequest(username);
//            List<GitHubResult> results = gitHubMapper.mapJsonToGitHubResultList(json);
    ...
    public Marker fetchAllReposNamesAndPrint (String username) {
        try {
            ...
            for (int i = 0; i < branchResult.size(); i++)
            {
                ...
                fetchAllBranchesForOneRepo("kalqa", branchResult.get(i).toString());
               // repoName.get(i);
            }
            //List<RepoName> reposNames = gitHubMapper.mapResultToRepoName(result);
